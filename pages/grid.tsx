import { NextPage } from 'next'
import { Grid, Center } from '@mantine/core'
import { Layout } from '../components/Layout'
import Link from 'next/link'
import { ReplyIcon } from '@heroicons/react/solid'

export const GridDemo: NextPage = ({}) => {
  return (
    <Layout title="Grid">
      <p>gridのテストページ</p>
      <p>gridは12でmax</p>
      <Grid my="md">
        <Grid.Col
          className="bg-blue-500 text-center font-bold text-gray-100"
          span={4}
        >
          1（span: 4）
        </Grid.Col>
        <Grid.Col
          className="bg-green-500 text-center font-bold text-gray-100"
          span={4}
        >
          2（span: 4）
        </Grid.Col>
        <Grid.Col
          className="bg-pink-500 text-center font-bold text-gray-100"
          span={4}
        >
          3（span: 4）
        </Grid.Col>
      </Grid>
      grow（足りない分の要素を伸ばす）
      <Grid my="md" grow>
        <Grid.Col
          className="bg-blue-500 text-center font-bold text-gray-100"
          span={6}
        >
          4（span: 6）
        </Grid.Col>
        <Grid.Col
          className="bg-green-500 text-center font-bold text-gray-100"
          span={6}
        >
          5（span: 6）
        </Grid.Col>
        <Grid.Col
          className="bg-pink-500 text-center font-bold text-gray-100"
          span={3}
        >
          6（span3だけど、Gridにgrowを付けるとはみ出して足りない分は伸びるよ）
        </Grid.Col>
        <Grid.Col
          className="bg-orange-500 text-center font-bold text-gray-100"
          span={3}
        >
          7（span3だけど、Gridにgrowを付けるとはみ出して足りない分は伸びるよ）
        </Grid.Col>
      </Grid>
      <p>offset</p>
      <Grid>
        <Grid.Col
          className="bg-blue-500 text-center font-bold text-gray-100"
          span={3}
        >
          8
        </Grid.Col>
        <Grid.Col
          className="bg-green-500 text-center font-bold text-gray-100"
          span={3}
        >
          9
        </Grid.Col>
        <Grid.Col
          className="bg-pink-500 text-center font-bold text-gray-100"
          span={3}
          offset={3}
        >
          10（offset: 3入れているよ）
        </Grid.Col>
      </Grid>
      <p>justify: center align: centerで上下中央寄せ</p>
      <Grid my="md" justify="center" align="center">
        <Grid.Col
          className="h-24 bg-blue-500 text-center font-bold text-gray-100"
          span={3}
        >
          11
        </Grid.Col>
        <Grid.Col
          className="h-36 bg-green-500 text-center font-bold text-gray-100"
          span={3}
        >
          12
        </Grid.Col>
        <Grid.Col
          className="h-12 bg-pink-500 text-center font-bold text-gray-100"
          span={3}
        >
          13
        </Grid.Col>
      </Grid>
      <p>justify: center align: flex-endで左右中央, 上下は下寄せ</p>
      <Grid my="md" justify="center" align="flex-end">
        <Grid.Col
          className="h-24 bg-blue-500 text-center font-bold text-gray-100"
          span={3}
        >
          11
        </Grid.Col>
        <Grid.Col
          className="h-36 bg-green-500 text-center font-bold text-gray-100"
          span={3}
        >
          12
        </Grid.Col>
        <Grid.Col
          className="h-12 bg-pink-500 text-center font-bold text-gray-100"
          span={3}
        >
          13
        </Grid.Col>
      </Grid>
      <p>
        ブレイクポイント: 基準はxx以上（例：md: 6とすると992px以上で反映される）
      </p>
      <p>
        最低レベルのブレイクポイントの値を下回ると1列になる（今回の例でいうと992px以下）
      </p>
      <Link
        className="text-blue-500"
        href="https://mantine.dev/core/media-query/#configure-breakpoints"
        target="_blank"
      >
        mantine: configure-breakpoints
      </Link>
      <Grid my="md">
        <Grid.Col
          className="bg-blue-500 text-center font-bold text-gray-100"
          md={6}
          lg={3}
        >
          14
        </Grid.Col>
        <Grid.Col
          className="bg-green-500 text-center font-bold text-gray-100"
          md={6}
          lg={3}
        >
          15
        </Grid.Col>
        <Grid.Col
          className="bg-pink-500 text-center font-bold text-gray-100"
          md={6}
          lg={3}
        >
          16
        </Grid.Col>
        <Grid.Col
          className="bg-orange-500 text-center font-bold text-gray-100"
          md={6}
          lg={3}
        >
          17
        </Grid.Col>
      </Grid>
      <Center>
        <Link href="/">
          <ReplyIcon className="mt-4 h-6 w-6 cursor-pointer text-gray-300" />
        </Link>
      </Center>
    </Layout>
  )
}

export default GridDemo
